...
@postUsers = (req,res) ->
  ...
  .then (u_v) ->
    service.acl.build_user(req.body, utils.get_ip_req(req))
  .then (response) ->
    @response = response
    res.json @response
  .catch (err) ->
...
@new_user = (req,res) ->
  ...
  .then (u_v) ->
    ...
    data.birthday = moment(req.body.birthday, 'YYYY-MM-DD').toDate()
    mongo.user data
    .saveAsync()
  .then (user_s) ->
  ...
  .then ( system_validation  ) ->
    if system_validation
      ...
      if config.verification.show_verif
        returnset.code = system_validation.number_code
    else
      returnset =
        status        : 200
        token         : @token_m.value
