...
@build_user = (params, ip) ->
  ...
  .then ( phone_v  ) ->
    ...
    if config.verification.show_verif
      r.code = @system_validation.number_code
...
@remove_sessions = remove_sessions = (force_removal, user   ) ->
  ...
  .then (stris) ->
    mongo.token.deleteManyAsync
      user   : user   
      token_type: $in: ['ANDROID', 'IOS', 'WEB']  
  .then (stris) ->
    return stris

@remove_social_login = remove_social_login = (force_removal, user   ) ->
  if not force_removal
    return 
  Promise.try ->
    mongo.refresh_token.deleteManyAsync
  ...
  .then (stris) ->
    mongo.token.deleteManyAsync
      user   : user   
      token_type: $in: ['FACEBOOK','GOOGLE','TWITTER']  
  .then (stris) ->
    return stris
